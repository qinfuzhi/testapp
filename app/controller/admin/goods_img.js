...
class GoodsImgController extends Controller {
  async indexPage() {
    const { ctx } = this;
    const data = await ctx.page('GoodsImg');
    console.log(data);
  ...
  async create() {
    ...
    if (await app.model.GoodsImg.findOne({ where: { name: data.name } })) {
      return ctx.apiFail(400, '该分类名称已经存在');
    }
    const shop = await app.model.GoodsImg.create(data);
    ctx.apiSuccess(shop);
  }
  // 编辑商品图片页面
  async editPage() {
    ...
    let data = await app.model.GoodsImg.findOne({
      where: { id },
    });
  ...
  async eidt() {
    const { app, ctx } = this;
    const GoodsImg = app.model.GoodsImg;
    const { id } = ctx.params;
    const data = ctx.request.body;
    // 验证码数据
    this.ctx.validate({
    ...
    // 判断id是否存在
    const shop = await app.model.GoodsImg.findOne({
      where: { id },
    });
    if (!shop) {
    ...
    const resone = await app.model.GoodsImg.findOne({
      where: {
        id: {
          [Op.ne]: id,
        },
    ...
    // 更新用户数据
    const res = await GoodsImg.update(data, { where: { id } });
    console.log(res);
    ctx.apiSuccess(res);
  }
  // 删除逻辑
  async delete() {
    ...
    await this.app.model.GoodsImg.destroy({
      where: {
        id,
      },
    });
...
module.exports = GoodsImgController;
